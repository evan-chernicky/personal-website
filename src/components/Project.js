...
function Project({project, index}) {
    ...
    <div className={`shadow-expand shadow-2xl dark:shadow-lg rounded-2xl overflow-hidden mx-4 my-8 md:m-8 ${isEven ? "dark:shadow-slate-700" : "dark:shadow-slate-700"}`}>
        <div className="border-white border-2 dark:border-0 relative rounded-t-2xloverflow-hidden">
            <div className={`rounded-t-2xl p-6 flex flex-col items-center justify-center absolute opacity-0 hover:opacity-100 bg-transparent w-full h-full z-10 ${isEven ? "hover:bg-pink/90" : "hover:bg-blue/90"}`}>
            ...
            {
                project.mp4 ? (
                <video className="w-full h-full rounded-t-2xl object-cover" autoPlay muted loop playsInline>  
                    {project.webm && <source src={project.webm} type="video/webm" />}                  
                    <source src={project.mp4} type="video/mp4"/>
                    Your browser does not support the video tag.
                </video> 
              ) : (
                <img className="w-full h-full rounded-t-2xl object-cover" src={project.image} alt={project.name} />
              )
            }
        </div>
        <div className="px-7 md:px-10 py-8 bg-white dark:bg-black mb-9">
            ...
            <div className="flex flex-wrap">
                {project.tech.map(single => (
                    <div key={single} className="py-1 px-3 bg-black m-1 rounded-full dark:bg-white">
